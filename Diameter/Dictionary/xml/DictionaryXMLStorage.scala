...
trait XMLDictionary {
  ...
  def locateAvps(elem:Elem) = {
    ...
    avpSeq.map((node) => {
      ...
      val typeName = {
        if ((node \ "grouped").nonEmpty)
          "Grouped"
        else if ((node \ "enum").nonEmpty)
          "Enumerated"
        else
          (node \ "type" \ "@type-name").text
      }
      ...
      //val avpVendorid = (node \ "@vendor-id").text match {case "" => None; case x => Some(x)}
      ...
      avpType match {
        case DictionaryAvpTypeValue.Grouped =>
          new DictionaryAvp(
            code = avpCode,
            name = avpName,
            typeName = avpType,
        ...
        case DictionaryAvpTypeValue.Enumerated =>
          new DictionaryAvp(
            code = avpCode,
            name = avpName,
            typeName = avpType,
